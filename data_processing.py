...
print('Fraudulent transaction type counts: ', df[df['isFraud'] == 1]['type'].value_counts())
# How many legitimate transactions are of type 'TRANSFER' or 'CASH_OUT'?
print('Number of legitimate transactions of type TRANSFER: ', len(df[(df['isFraud'] == 0) & (df['type'] == 'TRANSFER')]))
print('Number of legitimate transactions of type CASH_OUT: ', len(df[(df['isFraud'] == 0) & (df['type'] == 'CASH_OUT')]))
...
df_fraud_transfer_cashout = pd.merge(df_fraud_transfer, df_fraud_cashout, left_on='nameDest', right_on='nameOrig')
# How many fraudulent TRANSFER transactions are followed by a fraudulent CASH_OUT transaction?
print('Number of fraudulent TRANSFER transactions followed by a fraudulent CASH_OUT transaction: ',
...
print('Number of originators who have made more than 3 transactions: ', sum(nameOrig_counts_counts[4:]))
...
print('Number of fraudulent transactions made by originators who have made more than 1 transaction: ',
        len(df_multiple[df_multiple['isFraud'] == 1])) # not very many, 28/8213 (0.34%)
...
print('Fraction of fraudulent transactions with oldbalanceDest = 0 and newbalanceDest = 0 but amount is non-zero: ',
        len(df_fraud.loc[(df_fraud.oldbalanceDest==0) & (df_fraud.newbalanceDest==0) & (df_fraud.amount!=0)])
        / len(df_fraud))
print('Fraction of legitimate transactions with oldbalanceDest = 0 and newbalanceDest = 0 but amount is non-zero: ',
        len(df_legit.loc[(df_legit.oldbalanceDest==0) & (df_legit.newbalanceDest==0) & (df_legit.amount!=0)])
        / len(df_legit))
...
df.loc[(df.oldBalanceOrig==0) & (df.newBalanceOrig==0) & (df.amount!=0), ['oldBalanceOrig', 'newBalanceOrig']] = np.nan
...
ax.scatter(df[df['isFraud'] == 0]['errorBalanceDest'], df[df['isFraud'] == 0]['step'],
           -np.log10(df[df['isFraud'] == 0]['errorBalanceOrig'] + zOffset), c='b', marker='.', s=1,label='legitimate')
ax.scatter(df[df['isFraud'] == 1]['errorBalanceDest'], df[df['isFraud'] == 1]['step'],
           -np.log10(df[df['isFraud'] == 1]['errorBalanceOrig'] + zOffset), c='r', marker='.', s=1,label='fraudulent')
...
sns.heatmap(corrFraud, mask=mask, ax=ax1, vmin=-1, vmax=1, cmap='coolwarm', square=False, linewidths=.5, cbar=False)
ax1.set_title('Fraudulent\ntransactions')
ax1.set_xticklabels(ax1.get_xticklabels(), rotation=45)
ax1.set_yticklabels(ax1.get_yticklabels())
sns.heatmap(corrLegit, mask=mask, ax=ax2, vmin=-1, vmax=1, cmap='coolwarm', square=False, yticklabels=False,
            linewidths=.5, cbar_ax=cbar_ax, cbar_kws={'orientation': 'vertical', 'ticks': [-1, -0.5, 0, 0.5, 1]})
